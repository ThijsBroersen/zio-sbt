...
object ZioSbtCiPlugin extends AutoPlugin {
  override def requires: Plugins = plugins.CorePlugin && ZioSbtEcosystemPlugin
  ...
  object autoImport {
    ...
    val ciJobPerScalaPlatform: SettingKey[Boolean] =
      settingKey[Boolean]("Create a job for each Scala platform, default is true")
    val ciJobPerJavaVersion: SettingKey[Boolean] =
      settingKey[Boolean]("Create a job for each Java version, default is true")
    val ciJobPerScalaVersion: SettingKey[Boolean] =
      settingKey[Boolean]("Create a job for each Scala version, default is true")
    ...
    val ciTargetScalaPlatforms: SettingKey[Map[String, Seq[ScalaPlatform]]] =
      settingKey[Map[String, Seq[ScalaPlatform]]](
        "Scala platforms used for testing each module, the default value is an empty map which omits the test job on CI"
      )
  ...
  lazy val testJobs: Def.Initialize[Seq[Job]] = Def.settingDyn {
    Def.setting {
      ...
      ciTargetScalaPlatforms.?.value.getOrElse(Map.empty)
      val javaVersions       = autoImport.ciTargetJavaVersions.value
      val scalaVersions      = PlatformAssertions.Keys.allScalaVersions.value
      val scalaPlatforms     = PlatformAssertions.allScalaPlatforms.value
        ...
        Job(
          ...
          strategy = Some(
            Strategy(
              matrix = Map(
                "java"          -> javaVersions.toList,
                "scalaVersion"  -> scalaVersions.toList,
                "scalaPlatform" -> scalaPlatforms.toList.map(_.asString)
              ),
              maxParallel = matrixMaxParallel,
              failFast = false
            )
          ...
          steps = (if (swapSizeGB > 0) Seq(setSwapSpace) else Seq.empty) ++
            Seq(
              ...
              Step.SingleStep(
                name = "Test",
                run = Some(
                  prefixJobs + s"sbt ${crossbuildOrNot}test" ++ "${{ matrix.scalaPlatform }}${{ startsWith(matrix.scalaVersion, '2.12') && '2_12' || (startsWith(matrix.scalaVersion, '2.13') && '2_13' || (startsWith(matrix.scalaVersion, '3') && '3' || '')) }}"
                )
              )
            )
        )
  ...
  val IsSingleBuild: Def.Initialize[Boolean] = PlatformAssertions.scalaCrossVersionsCountsOnePerProject
