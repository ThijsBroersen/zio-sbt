...
object PlatformAssertions {
  ...
  trait Keys {
    ...
    lazy val defaultCrossScalaVersions: SettingKey[Seq[String]] =
      settingKey[Seq[String]]("Default cross scala versions")
  ...
  def allScalaPlatforms: Def.Initialize[Set[ScalaPlatform]] = Def.setting {
    Def.setting {
      Set(
        isScalaJS.value     -> ScalaPlatform.JS,
        isScalaJVM.value    -> ScalaPlatform.JVM,
        isScalaNative.value -> ScalaPlatform.Native
      ).collect { case (true, p) =>
        p
      }
    }.all(
  ...
  val docs: Def.Initialize[Seq[String]] = Def.settingDyn {
    val containsJS2_12                        = PlatformAssertions.containsJS2_12.value
    val containsJS2_13                        = PlatformAssertions.containsJS2_13.value
    val containsJS3                           = PlatformAssertions.containsJS3.value
    val containsJVM2_12                       = PlatformAssertions.containsJVM2_12.value
    val containsJVM2_13                       = PlatformAssertions.containsJVM2_13.value
    val containsJVM3                          = PlatformAssertions.containsJVM3.value
    val containsNative2_12                    = PlatformAssertions.containsNative2_12.value
    val containsNative2_13                    = PlatformAssertions.containsNative2_13.value
    val containsNative3                       = PlatformAssertions.containsNative3.value
    val allScalaCrossVersions                 = PlatformAssertions.allScalaCrossVersions.value.filter(_.size > 1).flatten
    val scalaCrossVersionsCountsOnePerProject = PlatformAssertions.scalaCrossVersionsCountsOnePerProject.value

    val activeScala212 =
      allScalaVersions.value.filter(_.startsWith("2.12")).toList.sorted.lastOption.getOrElse("2.12.x ")
    val activeScala213 =
      allScalaVersions.value.filter(_.startsWith("2.13")).toList.sorted.lastOption.getOrElse("2.13.x ")
    val activeScala3 = allScalaVersions.value.filter(_.startsWith("3.")).toList.sorted.lastOption.getOrElse("3.x  ")

    Def.setting(
      Seq(
        s"""
           | \u001b[33m### This build contains crossbuild projects (multiple scala versions defined in crossScalaVersions)\u001b[0m
           | \u001b[33mVersions found: ${allScalaCrossVersions.mkString(", ")}\u001b
        ...
        s"""
          ...
          | Current JDK ${if (currentJDK.value < javaPlatform.value) "\u001b[31m" else ""}${currentJDK.value}\u001b[0m
          | Target JDK ${javaPlatform.value} 
