...
import zio.sbt.SharedTasksAndSettings.autoImport.{
  banners,
  usefulTasksAndSettings,
  welcomeBannerEnabled,
  welcomeTaskAndSettingsEnabled
}
...
object ZioSbtEcosystemPlugin extends AutoPlugin {
  ...
  object autoImport extends ScalaCompilerSettings {
    ...
    def welcomeMessage: Setting[String] = ZioSbtEcosystemPlugin.welcomeMessage
  ...
  def welcomeMessage: Setting[String] =
    onLoadMessage := {
      if (welcomeBannerEnabled.value) {
        ...
        val allBanners                 = SharedTasksAndSettings.allBanners.value
        val allUserfulTasksAndSettings = SharedTasksAndSettings.allUserfulTasksAndSettings.value
        val maxLen                     = (allUserfulTasksAndSettings.map(_._1.length) ++ Seq(1)).max
        ...
        val renderedTasksAndSettings =
          if (welcomeTaskAndSettingsEnabled.value) {
            s"""|Useful sbt tasks:
                |${allUserfulTasksAndSettings.map { case (task, description) =>
